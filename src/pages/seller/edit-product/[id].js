...
function EditProduct() {
  ...
  const handleUpload = (e) => {
    if (e.target.files.length + selectedImages.length > 3)
      ...
      geProductId(productId)
        .then((res) => {
          ...
          if (res.data.data.images.length !== 0) {
            console.log("images is not empty", res.data.data.images.length);
            const images = res.data.data.images;
            console.log("images,", images);
            images.forEach((element, index) => {
              // const image = res.data.data.images[index];
              // console.log("image element", image, element);
              console.log("current image", element.image);
              const URL = process.env.NEXT_PUBLIC_API_URL;
              const imageBackend = URL  + element.image;
              const arr = selectedImages;
              arr.push(imageBackend);
              setSelectedImages(arr);
              console.log("this is image backend", imageBackend);
            });
          }
        })
        .catch((err) => {
  ...
  const renderPhotos = (photos) => {
    return photos.map((photo, index) => {
      console.log("render images", photo);
      return (
            ...
            <div className={styles["wrapper-main"]}>
              {category && brands ? (
                <>
                  ...
                  >
                    {selectedImages.length !== 0 && renderPhotos(selectedImages)}
                    <input
                      type="file"
                      id="file"
                      multiple
